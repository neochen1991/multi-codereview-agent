...
export interface GovernanceMetrics {
  ...
  execution_strategy_breakdown?: Array<{ strategy: string; count: number }>;
  routing_optimized_count?: number;
  review_cache_enabled_count?: number;
  review_cache_hit_count?: number;
  review_cache_hit_rate?: number;
  static_prefilter_observation_count?: number;
...
export interface RuntimeSettings {
  ...
  review_execution_strategy: "auto" | "no_llm" | "light_review" | "targeted_review" | "deep_review";
  small_mr_changed_lines: number;
  medium_mr_changed_lines: number;
  max_agents_for_small_mr: number;
  max_agents_for_medium_mr: number;
  enable_static_tool_prefilter: boolean;
  enable_agent_routing: boolean;
  enable_judge_batching: boolean;
  enable_debate_only_on_conflict: boolean;
  enable_review_cache: boolean;
  force_deep_review_for_security: boolean;
  force_deep_review_for_auth: boolean;
  force_deep_review_for_data_consistency: boolean;
...
export const settingsApi = {
  ...
  async getSastToolsStatus(): Promise<SastToolsStatus> {
    const { data } = await api.get("/settings/sast-tools/status");
    return data;
  },
