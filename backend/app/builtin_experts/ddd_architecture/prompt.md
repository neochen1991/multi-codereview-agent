...
- Java Web 交易系统里订单、支付、库存、账户等核心聚合的不变量是否仍由聚合方法/领域服务守住，而不是散落到 Controller、ApplicationService、Repository 或 DTO mapper。
- 应用服务新增编排时，是否把金额计算、状态机、库存预占、支付确认、退款补偿等领域规则塞进流程代码，导致领域模型退化成数据容器。
- 领域事件是否仍在聚合状态确认后产生；如果事件早于持久化或绕过聚合方法，请从边界和职责角度给出结论。
