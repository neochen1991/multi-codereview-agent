...
class ToolGatewayRepoTools:
    ...
    def __init__(
        self,
        *,
        diff_excerpt: DiffExcerptService,
        java_ddd_context_assembler: JavaDddContextAssembler,
        postgres_metadata: PostgresMetadataService,
    ) -> None:
        self._diff_excerpt = diff_excerpt
        self._java_ddd_context_assembler = java_ddd_context_assembler
        self._postgres_metadata = postgres_metadata

    def repo_context_enabled(self, runtime: RuntimeSettings, subject: ReviewSubject) -> bool:
    ...
    def repo_context_search(self, payload: dict[str, Any]) -> dict[str, Any]:
        MemoryProbe.log(
        ...
        related_files = [
            ...
            if str(item).strip() and str(item).strip() != file_path and self._is_source_like_path(str(item).strip())
        ]
        ...
        symbol_contexts = [
            self._filter_symbol_context(service.search_symbol_context(symbol, globs=None, definition_limit=2, reference_limit=4))
        ...
        derived_related_files = self._derive_related_files_from_anchors(ranked_anchors, primary_file_path=file_path)
        ...
        context_files = [item for item in [file_path, *effective_related_files[:3]] if item and self._is_source_like_path(item)]
    ...
    def pg_schema_context(self, payload: dict[str, Any]) -> dict[str, Any]:
        runtime = RuntimeSettings.model_validate(dict(payload.get("runtime") or {}))
        subject = ReviewSubject.model_validate(dict(payload.get("subject") or {}))
        file_path = str(payload.get("file_path") or "")
        line_start = int(payload.get("line_start") or 1)
    ...
    def transaction_boundary_inspector(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        java_context = self.extract_java_ddd_context(payload)
    ...
    def aggregate_invariant_inspector(self, payload: dict[str, Any]) -> dict[str, Any]:
        service = self._build_repository_context_service(payload)
        file_path = str(payload.get("file_path") or "").strip()
        line_start = int(payload.get("line_start") or 1)
        if not service.is_ready() or not file_path:
    ...
    def application_service_boundary_inspector(self, payload: dict[str, Any]) -> dict[str, Any]:
        service = self._build_repository_context_service(payload)
        file_path = str(payload.get("file_path") or "").strip()
        line_start = int(payload.get("line_start") or 1)
        if not service.is_ready() or not file_path:
    ...
    def controller_entry_guard_inspector(self, payload: dict[str, Any]) -> dict[str, Any]:
        service = self._build_repository_context_service(payload)
        file_path = str(payload.get("file_path") or "").strip()
        line_start = int(payload.get("line_start") or 1)
        if not service.is_ready() or not file_path:
        ...
        controller_contexts = [item for item in caller_contexts if isinstance(item, dict) and str(item.get("caller_type") or "").strip() == "controller"]
    ...
    def repository_query_risk_inspector(self, payload: dict[str, Any]) -> dict[str, Any]:
        service = self._build_repository_context_service(payload)
        file_path = str(payload.get("file_path") or "").strip()
        line_start = int(payload.get("line_start") or 1)
        if not service.is_ready() or not file_path:
    ...
    def extract_java_ddd_context(self, payload: dict[str, Any]) -> dict[str, Any]:
        service = self._build_repository_context_service(payload)
        file_path = str(payload.get("file_path") or "").strip()
        line_start = int(payload.get("line_start") or 1)
        related_files = [str(item).strip() for item in list(payload.get("related_files") or []) if str(item).strip() and str(item).strip() != file_path]
        subject = ReviewSubject.model_validate(dict(payload.get("subject") or {}))
        excerpt = self._diff_excerpt.extract_excerpt(str(subject.unified_diff or ""), file_path, line_start)
        primary_context = service.load_file_context(file_path, line_start, radius=20)
        ...
        symbol_contexts = [
            self._filter_symbol_context(service.search_symbol_context(symbol, globs=["*.java", "*.xml"], definition_limit=2, reference_limit=4))
            for symbol in symbol_queries[:3]
        ]
        if Path(file_path).suffix.lower() != ".java":
            return {}
    ...
    def java_ddd_runtime_tools_for_expert(self, expert_id: str, file_path: str) -> list[str]:
        ...
        mapping = {
            ...
            "ddd_architecture": ["application_service_boundary_inspector", "aggregate_invariant_inspector", "transaction_boundary_inspector"],
    ...
    def _extract_repo_search_terms(
        ...
        *,
        ...
        cleaned_lines = [re.sub(r"^\s*\d+\s+\|\s*[+\- ]?", "", line).strip() for line in str(excerpt or "").splitlines()]
        cleaned_excerpt = "\n".join(line for line in cleaned_lines if line)
        for token in self._extract_symbol_candidates(cleaned_excerpt):
            self._append_keyword_source(tokens, token_sources, token, source="diff_hunk", source_label="diff hunk")
        ...
        for token in self._extract_symbol_candidates(context_snippet):
            self._append_keyword_source(tokens, token_sources, token, source="source_context", source_label="源码上下文")
        file_stem_token = self._extract_symbol_from_file_name(file_path, allow_when_tokens_exist=bool(tokens))
        self._append_keyword_source(tokens, token_sources, file_stem_token, source="file_name", source_label="文件名类名兜底")
    ...
    def _extract_symbol_candidates(self, text: str) -> list[str]:
        ...
        for pattern in patterns:
            for match in re.findall(pattern, text):
                ...
                if not token:
                    continue
                if token.lower() in {"if", "for", "while", "switch", "return", "catch", "new", "throw", "else", "case", "try"}:
                    continue
    ...
    def _clean_repo_search_text(self, text: str) -> str:
        lines = [re.sub(r"^\s*\d+\s+\|\s*", "", line).strip() for line in str(text or "").splitlines()]
    ...
    def _append_keyword_source(
        ...
        *,
        ...
        token_sources.append({"keyword": normalized, "source": source, "source_label": source_label})
    ...
    def _filter_symbol_context(self, context: dict[str, Any]) -> dict[str, Any]:
        return {
            **context,
            "definitions": [item for item in list(context.get("definitions") or []) if isinstance(item, dict) and self._is_source_like_path(str(item.get("path") or ""))],
            "references": [item for item in list(context.get("references") or []) if isinstance(item, dict) and self._is_source_like_path(str(item.get("path") or ""))],
        }
    ...
    def _compact_symbol_entries(self, context: dict[str, Any], key: str) -> list[dict[str, Any]]:
        ...
        for item in list(context.get(key) or []):
            ...
            if not path or not self._is_source_like_path(path) or path in seen_paths:
                continue
            entries.append({"path": path, "line_start": int(item.get("line_number") or 1), "snippet": str(item.get("snippet") or "").strip()})
            seen_paths.add(path)
        return entries
    ...
    def _build_related_source_snippets(
        ...
        *,
        ...
        for anchor in ranked_anchors:
            ...
            if not path or path == primary_file_path or path in best_anchor_by_path or not self._is_source_like_path(path):
                continue
            best_anchor_by_path[path] = anchor
        for anchor in best_anchor_by_path.values():
            path = str(anchor.get("path") or "").strip()
            if not path or path == primary_file_path or path in seen_paths or not self._is_source_like_path(path):
                continue
            ...
            if not snippet:
                continue
            snippets.append({"path": path, "symbol": str(anchor.get("symbol") or "").strip(), "kind": str(anchor.get("kind") or "reference").strip(), "line_start": line_number, "snippet": snippet})
            seen_paths.add(path)
            if len(snippets) >= max_snippets:
                return snippets
        for related_path in list(related_files or []):
            normalized_path = str(related_path or "").strip()
            if not normalized_path or normalized_path == primary_file_path or normalized_path in seen_paths or not self._is_source_like_path(normalized_path):
                continue
            line_number = self._pick_related_context_line(service, normalized_path, symbol_contexts, symbol_priorities=symbol_priorities)
            context_snippet = self._load_related_context_window(service, normalized_path, line_number)
            snippet = str(context_snippet.get("snippet") or "").strip()
            if not snippet:
                continue
            snippets.append({"path": normalized_path, "symbol": Path(normalized_path).stem, "kind": "reference", "line_start": line_number, "snippet": snippet})
    ...
    def _load_related_context(
        ...
        *,
        ...
        line_number = self._pick_related_context_line(service, related_path, symbol_contexts, symbol_priorities=symbol_priorities)
        return self._load_related_context_window(service, related_path, line_number)

    def _load_related_context_window(self, service: RepositoryContextService, related_path: str, line_number: int) -> dict[str, Any]:
        ...
        return {"path": related_path, "snippet": snippet, "line_start": normalized_line, "line_end": normalized_line}
    ...
    def _pick_related_context_line(
        ...
        *,
        ...
        normalized_path = str(related_path or "").strip()
        best_anchor = self._select_best_anchor_for_path(normalized_path, symbol_contexts, symbol_priorities=symbol_priorities or {})
        if best_anchor:
        ...
        if file_symbol:
            ...
            fallback_anchor = self._select_best_anchor_for_path(normalized_path, [symbol_context], symbol_priorities=symbol_priorities or {})
            if fallback_anchor:
        ...
        for context in symbol_contexts:
            ...
            for raw_kind in ("definitions", "references"):
                ...
                for item in list(context.get(raw_kind) or []):
                    ...
                    anchors.append({"symbol": symbol, "kind": kind, "path": path, "line_number": line_number, "snippet": snippet, "score": score})
        anchors.sort(key=lambda item: (-int(item.get("score") or 0), str(item.get("path") or ""), int(item.get("line_number") or 1)))
        return anchors
    ...
    def _select_best_anchor_for_path(
        ...
        *,
        ...
        anchors = [
            item
            for item in self._collect_ranked_symbol_anchors(symbol_contexts, symbol_priorities=symbol_priorities or {})
    ...
    def _looks_like_java_call_site(self, snippet: str, *, symbol: str) -> bool:
        ...
        if not normalized:
            return False
        escaped_symbol = re.escape(str(symbol or "").strip())
        return bool(re.search(rf"(?:\.|\b){escaped_symbol}\s*\(", normalized) or re.search(rf"\bnew\s+{escaped_symbol}\s*\(", normalized))
