    ...
    def run_once(self, review_id: str) -> ReviewTask:
        ...
        issues = [
            DebateIssue(
                ...
                change_understanding_refs=self._normalize_text_list(item.get("change_understanding_refs"), []),
        ...
        if not issues and eligible_fallback_candidates:
            ...
            issues = [
                DebateIssue(
                    ...
                    change_understanding_refs=self._normalize_text_list(
                        fallback_source.get("change_understanding_refs"),
                        [],
                    ),
        ...
        **_: object,
        ...
        for index, raw_parsed in enumerate(parsed_candidates, start=1):
            ...
            finding = ReviewFinding(
                ...
                file_path=finding_file_path,
                line_start=parsed_line_start,
                method_name=str(parsed.get("method_name") or "").strip(),
                code_anchor=str(parsed.get("code_anchor") or "").strip(),
                change_understanding_refs=self._normalize_text_list(parsed.get("change_understanding_refs"), []),
                        ...
                        "candidate_findings": [
                            {
                                ...
                                "change_understanding_refs": ["target_id、文件路径、类名或方法名"],
                                "evidence": "string",
