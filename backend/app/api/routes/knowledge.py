...
def list_knowledge() -> list[dict[str, object]]:
    return [item.model_dump(mode="json") for item in review_service.list_knowledge()]


@router.get("/knowledge/grouped")
def list_grouped_knowledge() -> dict[str, list[dict[str, object]]]:
    grouped: dict[str, list[dict[str, object]]] = {}
    for item in review_service.list_knowledge():
...
def retrieve_knowledge(
    ...
    return [
        ...
        for item in review_service.retrieve_knowledge(expert_id, {"changed_files": changed_files})
    ]
...
def create_knowledge_doc(payload: CreateKnowledgeDocumentRequest) -> dict[str, object]:
    document = review_service.create_knowledge_document(payload.model_dump())
