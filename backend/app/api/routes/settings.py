...
def _sast_tool_status() -> dict[str, object]:
    ...
    repository = runtime.resolve_repository(
        repository_id=str(runtime.default_repository_id or "").strip(),
        project_id=str(runtime.default_project_id or "").strip(),
    )
    repo_root = str(repository.local_path if repository is not None else runtime.code_repo_local_path or "").strip()
    return SastPreScanService().tool_status(
...
class RuntimeSettingsRequest(BaseModel):
    ...
    small_mr_changed_lines: int = 80
    medium_mr_changed_lines: int = 400
    max_agents_for_small_mr: int = 2
    max_agents_for_medium_mr: int = 3
